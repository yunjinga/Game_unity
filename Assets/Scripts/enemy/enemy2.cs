...
public class enemy2 : MonoBehaviour
{
    ...
    private Vector3 rotation;//初始旋转值
    public bool isGeneraterotation = false;//是否要生成旋转值
    private Mesh mesh;//mesh
    public float distance_normal = 5, distance_waring = 7, distance;//警戒前后获得气息值的范围
    private Vector3[] vertices;//mesh的vertices
    public Vector3 v1;
    public float waitTurnTime = 10f;//等待转向时间
    public float TurnAngle = 180; //旋转角度
    ...
    void Update()
    {
        ...
        if (!isWalkToOink && !isLook && !ator.GetBool("isAttack") && !ator.GetBool("isRun") && !ator.GetBool("isWalk"))
        {
            if (waitTurnTime >= 0) waitTurnTime -= Time.deltaTime;
            else
            {
                if (!isGeneraterotation)
                {
                    rotation = new Vector3(0, TurnAngle, 0);
                    rotation += transform.localEulerAngles;
                    TurnAngle = -TurnAngle;
                    isGeneraterotation = true;
                }
                turnAround(rotation);
    ...
    void turnAround(Vector3 rotation)
    {
        
        targetPoint = Quaternion.Euler(rotation);
        transform.rotation = Quaternion.Slerp(transform.rotation, targetPoint, 2 * Time.deltaTime);//平滑转向
